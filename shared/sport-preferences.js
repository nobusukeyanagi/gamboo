(() => {
  ...
  if (window.GAMBOO_SPORT_PREFERENCES) return;

  const STORAGE_KEY = "gamboo:sport-preferences:v1";
  ...
  const notify = (state, source = "local") => {
    window.dispatchEvent(new CustomEvent("gamboo:sport-preferences-change", {
      detail: {
  ...
  window.GAMBOO_SPORT_PREFERENCES = Object.freeze({
    STORAGE_KEY,
    SPORTS,
    SPORT_IDS,
    read,
