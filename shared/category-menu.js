      ...
      shadow.innerHTML = `
        <style>
          *{box-sizing:border-box}
          ...
          nav{min-width:0;height:100%;display:flex;align-items:stretch;justify-content:flex-end;gap:12px}
          nav a{position:relative;min-width:82px;display:flex;align-items:center;justify-content:center;padding:0 12px;color:#5c4514;text-decoration:none;font-size:15px;font-weight:800;line-height:1;letter-spacing:.015em;white-space:nowrap;-webkit-tap-highlight-color:transparent;text-shadow:none}
          ...
          @media(max-width:899px){.category-bar{grid-template-columns:1fr;grid-template-rows:34px 42px;padding:0 2px}.brand{padding:0;border-bottom:1px solid rgba(213,171,67,.34)}.brand img{width:auto;height:34px;max-width:min(160px,46vw)}nav{grid-row:2;display:grid;grid-template-columns:repeat(${ITEMS.length},minmax(0,1fr));column-gap:0}nav a{min-width:0;font-size:14px;padding-inline:0;letter-spacing:0}}
          @media(max-width:350px){.brand img{max-width:145px}nav a{font-size:12.5px;letter-spacing:-.02em}}
        </style>
        <div class="category-bar">
          <a class="brand" href="${homeUrl.href}" target="_blank" rel="noopener noreferrer" aria-label="Gambooトップへ（新しいタブで開く）">
