(() => {
  ...
  document.addEventListener("gamboo:before-vote-navigation", persistCurrentRaceSelection);

  class GambooRaceInfo extends HTMLElement {
    connectedCallback() {
      ...
      const applyVenueFilter = () => {
        ...
        const raceSwitch = document.querySelector('gamboo-race-switch');
        ...
        document.dispatchEvent(new CustomEvent('gamboo:race-venue-filter', { detail }));
      };
      ...
      document.addEventListener('gamboo:vote-race-state-refresh', restoreRaceSelection);
  ...
  if (!customElements.get("gamboo-race-info")) {
    customElements.define("gamboo-race-info", GambooRaceInfo);
  }
})();
